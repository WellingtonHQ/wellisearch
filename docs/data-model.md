...
| `query` | `text` | |
| `source` | `text` | `local` \| `tavily` \| `brave` \| `exa` \| `youcom` |
...
| Column | Type | Notes |
|---|---|---|
| `provider` | `text` **PK** | `tavily` \| `brave` \| `exa` \| `youcom` |
...
(`TAVILY_QUOTA_MONTHLY` / `BRAVE_QUOTA_MONTHLY` / `EXA_QUOTA_MONTHLY` / `YOUCOM_QUOTA_MONTHLY`).
